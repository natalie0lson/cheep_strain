#!/bin/bash
#SBATCH --job-name=straingr_cheep_focal
#SBATCH --nodes=1
#SBATCH --account=mnadimp
#SBATCH --tasks-per-node=1
#SBATCH --cpus-per-task=8
#SBATCH --output=log/straingr_cheep_focal_%a.out
#SBATCH --error=log/straingr_cheep_focal_%a.err
#SBATCH --array=1-47
...
# ── Paths ──────────────────────────────────────────────────────────────────────
PAIRS_TSV=/projects/mnadimlab/nmolson/scripts/strainGE/03-straingr/05-all_commands/cheep_focal_shared_ref_pairs.tsv
