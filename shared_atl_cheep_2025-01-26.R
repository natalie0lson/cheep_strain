...
ST155 <- read.csv("Data/ATL_CHEEP_Strains/atl_13C1079T.csv")
STD4 <- read.csv("Data/ATL_CHEEP_Strains/atl_D4.csv")
ST224 <- read.csv("Data/ATL_CHEEP_Strains/atl_KW21.csv")
head(ST155)
human_ids <- read.csv("Data/atl_ids.csv")

ST155$strain <- "13C1079T"
STD4$strain <- "D4"
ST224$strain <- "KW21"

all_strains <- rbind(ST155, STD4, ST224)
...
shared$human_id <- NA
shared$human_id[shared$sample1 %in% human_ids$ID] <- shared$sample1[shared$sample1 %in% human_ids$ID]
shared$human_id[shared$sample2 %in% human_ids$ID] <- shared$sample2[shared$sample2 %in% human_ids$ID]
table(shared$human_id)
...
101*60
table(shared$threshold[shared$strain=="13C1079T"])
32/6060
6/6060
2/6060

101+177+62
table(shared$threshold[shared$strain=="D4"])
table(shared$threshold[shared$strain=="KW21"])
shared
...
human_ids$human_id <- human_ids$ID
all_ids <- merge(shared, human_ids, by="human_id", all=TRUE)
head(all_ids)
...
all_D4 <- subset(all_ids, all_ids$strain=="D4")
all_KW21 <- subset(all_ids, all_ids$strain=="KW21")
...
matrix_D4 <- all_D4[variables]
matrix_KW21 <- all_KW21[variables]

matrix$threshold[is.na(matrix$threshold)] <- 0
matrix_155$threshold[is.na(matrix_155$threshold)] <- 0
matrix_D4$threshold[is.na(matrix_D4$threshold)] <- 0
matrix_KW21$threshold[is.na(matrix_KW21$threshold)] <- 0
...
### Site-Level Comparison (Production, Market, Processing)
  ...
  # Apply the permutation test
  p_value_strains <- permutation.test.multi(as.vector(mat155), as.vector(mat10), as.vector(mat117), as.vector(mat540))
